#!/usr/bin/env python
#### slightly modified from 'feature_extract_batchprocessing_parallel.py' from `pyradiomics`` package ####
### 
...
#### outputs ####
out_dir = os.path.join(dat_dir,'radiomics')
...
def run(case):
  global PARAMS,out_dir, TEMP_DIR
  ...
  try:
    ...
    output_filename = os.path.join(out_dir, TEMP_DIR, filename)
...
if __name__ == '__main__':
  ...
  cases_file = os.path.join(out_dir, 'cases_paths.csv')

  csv_file = open(cases_file, 'w',encoding='utf-8-sig')
  csv_writer = csv.writer(csv_file, delimiter=',')
  csv_writer.writerow(['pat_id', 'Image', 'Mask',"Label"])
  csv_file.flush()
  #TBD

  #### create a spreadsheet storing images and labels file paths ####

  case_list = [i.split("_")[0] for i in os.listdir(os.path.join(dat_dir,"mask"))]
  for pat_id in case_list:
    img_file = os.path.join(dat_dir,"image", '{}_CT.nii.gz'.format(pat_id))
    lab_file = os.path.join(dat_dir, "mask", '{}_CT_Segment.nii.gz'.format(pat_id))
    csv_writer.writerow([pat_id, img_file, lab_file, "nan"])
    csv_file.flush()
  csv_file.close()
  ...
  # File variables
  PARAMS = os.path.join('feature_extract_params.yaml')  # Parameter file
  LOG = os.path.join(out_dir, 'log.txt')  # Location of output log file
  INPUTCSV = cases_file # os.path.join(ROOT, 'data', 'cases_paths.csv')
  OUTPUTCSV = os.path.join(out_dir, 'Omics_Feats_CT.csv')
  ...
  if not os.path.isdir(os.path.join(out_dir, TEMP_DIR)):
    logger.info('Creating temporary output directory %s', os.path.join(out_dir, TEMP_DIR))
    os.mkdir(os.path.join(out_dir, TEMP_DIR))
  ...
  try:
    ...
    if REMOVE_TEMP_DIR:
      logger.info('Removing temporary directory %s (contains individual case results files)',
                  os.path.join(out_dir, TEMP_DIR))
      shutil.rmtree(os.path.join(out_dir, TEMP_DIR))
  except Exception:
